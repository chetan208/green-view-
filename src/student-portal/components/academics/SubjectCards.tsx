...
export default function SubjectCards({ subjects }: { subjects: any[] }) {
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-[0_1px_2px_0_rgba(0,0,0,0.02)] flex flex-col gap-5">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="flex flex-col select-none">
                    <span className="text-xs font-bold text-slate-750 tracking-tight">{sub.name}</span>
                    <span className="text-[9px] text-slate-400 font-semibold mt-0.5">{sub.remarks}</span>
                  </div>
                </div>
                <div className="flex flex-col items-end select-none">
                  <span className="text-xs font-extrabold text-slate-750 font-mono">{sub.marks} / {sub.maxMarks}</span>
                  <span className="text-[8px] font-bold text-brand-green mt-1 bg-emerald-50 border border-emerald-100/30 px-1.5 py-0.5 rounded-md uppercase tracking-wider">{sub.grade}</span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1">
                ...
                {sub.classAvg && (
                  <div className="flex items-center justify-between text-[9px] font-bold text-slate-400 mt-1 select-none">
                    ...
                    <span>Diff: <span className="text-brand-green">+{sub.marks - sub.classAvg}%</span></span>
                  </div>
                )}
              </div>
            </div>
