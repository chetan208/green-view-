...
export default function StaffManager() {
  ...
  return (
    <div className="space-y-6 text-slate-800">
      ...
      {/* Header */}
      <div>
        <h2 className="text-xl sm:text-2xl font-bold font-serif text-slate-900">Human Resources</h2>
        <p className="text-xs font-semibold text-slate-500 mt-1">Manage employee records, roles, and contact information.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Teaching Faculty</p>
            <h3 className="text-2xl font-black text-slate-900">{teachingStaff}</h3>
          </div>
        </div>
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex items-center gap-4">
          ...
          <div>
            <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Administration</p>
            <h3 className="text-2xl font-black text-slate-900">{adminStaff}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
        ...
        {!showForm && (
          <button 
            ...
            className="w-full md:w-auto bg-brand-green hover:bg-brand-green-dark text-white px-5 py-2.5 rounded-lg text-sm font-bold flex items-center justify-center gap-2 transition shadow-sm border-0 cursor-pointer"
          >
        ...
        {showForm && (
          ...
          >
            <div className="flex justify-between items-center border-b border-slate-100 pb-4">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="text-xs font-bold text-slate-400 hover:text-slate-600 transition bg-slate-100 hover:bg-slate-200 px-3 py-1.5 rounded-md border-0 cursor-pointer">Cancel</button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
              {/* Photo Upload Area */}
              <div className="md:col-span-3 flex flex-col gap-1.5">
                <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400">Profile Photo</label>
                <label className="flex flex-col items-center justify-center w-full aspect-square border-2 border-slate-200 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-brand-green/5 hover:border-brand-green/30 transition-all group">
                  ...
                  <p className="text-[10px] font-bold text-slate-500 text-center px-4">Upload Headshot</p>
                </label>
              </div>
              ...
              <div className="md:col-span-9 grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5">Role / Designation *</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5">Department *</label>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1.5">Date of Joining</label>
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button type="submit" className="w-full sm:w-auto bg-slate-900 hover:bg-slate-800 text-white px-8 py-2.5 rounded-lg text-sm font-bold transition flex items-center justify-center shadow-md border-0 cursor-pointer">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="px-6 py-3 text-[10px] font-black uppercase tracking-wider text-slate-500 whitespace-nowrap">Employee</th>
                <th className="px-6 py-3 text-[10px] font-black uppercase tracking-wider text-slate-500 whitespace-nowrap">Role & Dept</th>
                <th className="px-6 py-3 text-[10px] font-black uppercase tracking-wider text-slate-500 whitespace-nowrap">Contact</th>
                <th className="px-6 py-3 text-[10px] font-black uppercase tracking-wider text-slate-500 whitespace-nowrap text-right">Actions</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900 leading-none mb-1">{staff.name}</p>
                          <p className="text-[10px] font-black text-brand-green uppercase tracking-wider">{staff.id}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <select 
                        ...
                        className="text-sm font-bold text-slate-700 bg-transparent border-b border-dashed border-slate-300 hover:border-brand-green focus:border-brand-green focus:outline-none cursor-pointer pb-0.5 mb-1 block w-fit"
                      >
                      ...
                      <p className="text-xs font-semibold text-slate-400">{staff.department}</p>
                    </td>
                    <td className="px-6 py-4">
          ...
          {filteredStaff.length === 0 && (
            <div className="w-full bg-white p-12 flex flex-col items-center justify-center text-center border-t border-slate-100">
              ...
              <h3 className="text-sm font-bold text-slate-800 mb-1">No Records Found</h3>
              <p className="text-xs font-semibold text-slate-500">Try adjusting your search criteria.</p>
            </div>
          )}
        </div>
      </div>
