      ...
      <div className={`max-w-7xl mx-auto w-full px-6 md:px-8 flex items-center justify-between transition-all duration-300 ${isScrolled ? "py-2.5" : "py-4"}`}>
        ...
        <Link href="/" className="flex items-center gap-3 shrink-0">
          ...
          <div className="flex flex-col">
            <span className="font-black text-brand-navy text-base md:text-lg leading-none tracking-tight">
              Green view
            </span>
            <span className="text-[7px] text-brand-navy font-black tracking-widest uppercase mt-1.5 leading-none">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col">
                            <span className="text-[11px] font-black text-slate-800 group-hover/item:text-brand-green transition-colors leading-normal uppercase tracking-wider">
                              {subItem.name}
                            </span>
                            {subItem.desc && (
                              <span className="text-[9px] text-slate-400 font-bold mt-1 leading-none normal-case">
                                {subItem.desc}
                              </span>
                            )}
                          </div>
                ...
                {item.hasDropdown && item.dropdownItems ? (
                  <div className="flex flex-col gap-1">
                    <button 
                      ...
                      className="flex items-center justify-between text-xs font-black text-slate-700 hover:text-brand-green py-2 border-b border-slate-50 transition-colors uppercase tracking-wider w-full text-left"
                    >
                      {item.name}
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <div className="flex-grow flex flex-col">
                                    <span className="text-[10px] font-extrabold text-slate-500 hover:text-brand-green uppercase tracking-wider block leading-none">
                                      {subItem.name}
                                    </span>
                                    {subItem.desc && (
                                      <span className="text-[8.5px] text-slate-400 font-bold block mt-1.5 normal-case leading-none">
                                        {subItem.desc}
                                      </span>
                                    )}
                                  </div>
                  ...
                  <Link
                    href={item.href}
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="text-xs font-black text-slate-700 hover:text-brand-green py-2 border-b border-slate-50 transition-colors uppercase tracking-wider block"
                  >
                    {item.name}
                  </Link>
                )}
