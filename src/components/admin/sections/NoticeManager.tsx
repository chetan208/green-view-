      ...
      {showForm && (
        <form className="bg-white border border-slate-200 rounded-xl p-5 sm:p-6 shadow-sm space-y-5 animate-in fade-in slide-in-from-top-2 duration-150">
          <div className="flex justify-between items-center border-b border-slate-100 pb-3">
            <h3 className="text-sm font-semibold text-slate-900">{editingId ? "Edit Notice Details" : "Create New Notice"}</h3>
          ...
          <div>
            <label className="block text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-1.5">Attach PDF Document (Optional)</label>
            <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-slate-200 border-dashed rounded-xl cursor-pointer bg-slate-50 hover:bg-slate-100/50 transition-all">
              ...
              <p className="text-xs font-semibold text-slate-600">Click to upload document</p>
            </label>
          </div>

          <button type="button" onClick={resetForm} className="w-full bg-brand-green hover:bg-brand-green-dark text-white py-2.5 rounded-lg text-sm font-semibold transition flex items-center justify-center gap-1.5 shadow-sm border-0 cursor-pointer">
            Publish Notice
          </button>
        </form>
      )}
