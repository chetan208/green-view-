    ...
    <section className="w-full py-16 md:py-24 bg-slate-50 flex justify-center border-t border-b border-slate-100">
      <div className="max-w-7xl w-full px-6 md:px-12 lg:px-16 flex flex-col gap-12 md:gap-16">
        ...
        <div className="flex flex-col items-center text-center gap-3">
          <span className="text-brand-green font-medium md:font-semibold text-xs uppercase tracking-[0.25em]">
            CHOOSE YOUR PATH
          </span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-medium md:font-bold text-slate-900 tracking-tight leading-tight max-w-3xl">
              ...
              >
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-lg md:text-xl font-medium md:font-bold text-slate-800 tracking-tight">
                      {stream.title}
                    </h3>
                  </div>
                  ...
                  <div className="flex flex-col gap-2.5">
                    <h4 className="text-slate-800 font-medium md:font-semibold text-xs uppercase tracking-wide">
                      Core Subjects
                    </h4>
                    <div className="flex flex-wrap gap-2">
                      {stream.coreSubjects.map((sub, sIdx) => (
                        <span key={sIdx} className="bg-slate-50 text-slate-600 text-[10px] md:text-xs font-medium md:font-semibold px-3 py-1 rounded-lg border border-slate-100/50">
                          {sub}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {stream.electives.map((sub, sIdx) => (
                        <span key={sIdx} className="bg-slate-50/50 text-slate-500 text-[10px] md:text-xs font-normal md:font-medium px-2.5 py-0.5 rounded-lg border border-dashed border-slate-200">
                          {sub}
                        </span>
                      ))}
                    </div>
