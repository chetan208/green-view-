...
export default function PrimaryTeachers() {
  ...
  const fetchTeachers = async (pageToFetch: number, isInitial = false) => {
    try {
      ...
      const res = await erpApi.teachers.list({ category: 'primary', isTeacher: true, page: pageToFetch, limit: 12 });
      ...
      console.error("Failed to fetch primary teachers:", error);
    } finally {
      setLoading(false);
  ...
  return (
    <section className="w-full py-10 md:py-16 bg-slate-50">
      <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16">
        ...
        >
          Primary Section <span className="text-brand-green">Teachers</span>
        </motion.h2>
