    ...
    <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-16 py-8 flex flex-col gap-8 min-h-[500px]">
          ...
          <div className="text-xs md:text-sm font-medium md:font-semibold text-slate-500 flex items-center gap-2">
            ...
            {selectedClass && (
              <>
                ...
                <span className={`${!selectedSubject ? "text-slate-800 font-medium md:font-bold" : "text-brand-green cursor-pointer hover:underline"}`} onClick={() => selectedSubject && handleBack()}>
                  {selectedClass.name}
                </span>
              </>
            )}
            {selectedSubject && (
              <>
                <span className="text-slate-300">/</span>
                <span className="text-slate-800 font-medium md:font-bold">{selectedSubject.name}</span>
              </>
            )}
          </div>
        </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {!selectedClass && (
          ...
          >
            <h2 className="text-xl font-medium md:font-bold text-slate-800 tracking-tight">Select Class:</h2>
              ...
              {studyData.map((cls, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="text-lg font-medium md:font-bold text-slate-800 mb-2 group-hover:text-brand-green transition-colors">{cls.name}</h3>
                    <p className="text-slate-500 text-xs md:text-sm font-normal md:font-medium leading-relaxed">{cls.desc}</p>
                  </button>
                );
              })}
            </div>
        ...
        {selectedClass && !selectedSubject && (
          <motion.div
            ...
            className="flex flex-col gap-6"
          >
            <h2 className="text-xl font-medium md:font-bold text-slate-800 tracking-tight">
              ...
              {selectedClass.subjects.map((sub, idx) => (
                ...
                >
                  ...
                  <div className="flex flex-col">
                    <span className="text-base font-medium md:font-bold text-slate-800 group-hover:text-brand-green transition-colors">{sub.name}</span>
        ...
        {selectedClass && selectedSubject && (
          ...
          >
            ...
            <div className="w-full flex justify-start md:justify-center border-b border-slate-100 pb-3">
              <div className="flex overflow-x-auto no-scrollbar max-w-full gap-1 md:gap-2 p-1 bg-slate-100/80 rounded-2xl flex-nowrap">
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 px-4 py-2.5 md:px-6 md:py-3 rounded-xl font-medium md:font-semibold text-[10px] md:text-sm transition-all cursor-pointer whitespace-nowrap ${
                    activeTab === "notes"
            ...
            <div className="flex flex-col gap-4">
              <h2 className="text-lg font-medium md:font-bold text-slate-800 tracking-tight flex items-center gap-2 capitalize">
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col gap-1">
                        ...
                        <h3 className="font-medium md:font-bold text-slate-800 text-sm md:text-base leading-snug">
                          {chapter.name}
                        </h3>
                      </div>
                      ...
                      {activeTab === "notes" && (
                        <div className="flex items-center justify-between border-t border-slate-50 pt-4 mt-1">
                          <span className="text-[10px] font-medium md:font-semibold text-slate-400 uppercase">
                            File Size: {chapter.materials.notesSize}
                          </span>
                          <button
                            onClick={() => alert(`Downloading Notes: ${chapter.name}`)}
                            className="inline-flex items-center gap-1.5 bg-brand-green/10 hover:bg-brand-green text-brand-green hover:text-white px-4 py-2 rounded-xl text-xs font-medium md:font-semibold transition-all cursor-pointer"
                          >
                      ...
                      {activeTab === "lectures" && (
                        <div className="flex flex-col border-t border-slate-50 pt-4 mt-1 gap-3">
                          <div className="flex items-center justify-between">
                            <span className="text-[9px] font-medium md:font-semibold text-slate-400 uppercase truncate max-w-[150px]">
                              {chapter.materials.videoTitle}
                            </span>
                            <button
                              onClick={() => setActiveVideoId(isVideoActive ? null : chapter.id)}
                              className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium md:font-semibold border transition-all cursor-pointer ${
                                isVideoActive
                            ...
                            {isVideoActive && (
                              ...
                              >
                                <div className="bg-slate-900 aspect-video w-full rounded-xl flex flex-col items-center justify-center text-white p-4 border border-slate-800 shadow-inner relative">
                                  ...
                                  <h4 className="font-medium md:font-bold text-xs text-center leading-snug px-2 mb-1">
                                    {chapter.materials.videoTitle}
                                  </h4>
                                  <p className="text-[8px] text-slate-400 font-medium md:font-semibold uppercase tracking-wider text-center">
                                    Demo Stream Player
                                  </p>
                                </div>
                              </motion.div>
