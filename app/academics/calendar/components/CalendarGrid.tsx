      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-100 pb-6">
        {categories.map(cat => (
          <button
            ...
            className={`px-4 py-2 rounded-full font-medium md:font-semibold text-xs md:text-sm transition-all ${
              activeTab === cat
          ...
          return (
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center gap-6">
                {/* Date Box */}
                <div className="w-24 bg-emerald-50 text-brand-green font-medium md:font-semibold p-3 rounded-xl flex flex-col items-center justify-center text-center shrink-0 border border-emerald-100/50">
                  <span className="text-[9px] uppercase tracking-wider mb-0.5 leading-none">Date</span>
                  <span className="text-xs font-medium md:font-bold leading-normal whitespace-nowrap">{dateParts[0]} {dateParts[1]}</span>
                  <span className="text-[9px] text-emerald-600 font-medium md:font-bold mt-0.5 leading-none">{dateParts[2]}</span>
                </div>
                
                {/* Details */}
                <div className="flex flex-col">
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className={`text-[9px] font-medium md:font-bold uppercase tracking-wider px-2 py-0.5 rounded-md ${
                    ...
                    <span className="flex items-center gap-1 text-[10px] font-medium md:font-semibold text-slate-400">
                      <Clock className="w-3 h-3" /> {ev.time}
                    </span>
                  </div>
                  <h3 className="font-medium md:font-bold text-slate-800 text-sm md:text-base group-hover:text-brand-green transition-colors leading-snug">
                    {ev.title}
                  </h3>
