    ...
    <div className="w-full bg-white rounded-2xl border border-slate-200 shadow-sm p-5 md:p-8 mb-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-10">
        ...
        <div className="md:col-span-3 flex flex-col items-center md:items-start">
          <label className="text-sm font-medium md:font-semibold text-slate-800 mb-3 block">
            Student Photograph *
          </label>
          <label className={`w-40 h-48 rounded-2xl border-2 border-dashed flex flex-col items-center justify-center cursor-pointer transition-colors group relative overflow-hidden ${hasPhotoError ? 'border-red-400 bg-red-50 hover:bg-red-100' : 'border-emerald-300 bg-emerald-50/30 hover:bg-emerald-50'}`}>
            ...
            {photoPreview ? (
              ...
              <>
                ...
                <span className={`text-sm font-medium md:font-semibold ${hasPhotoError ? 'text-red-500' : 'text-brand-green'}`}>Upload Photo</span>
                <span className="text-[10px] font-medium text-slate-400 mt-1">JPG, PNG up to 2MB</span>
              </>
            )}
          </label>
          {hasPhotoError && <span className="text-[10px] font-medium md:font-semibold text-red-500 mt-2">Photo is mandatory</span>}
        ...
        <div className="md:col-span-9 flex flex-col gap-8">
              ...
              <div className="flex gap-4">
                {["Class 11", "Class 12"].map((cls) => (
                  ...
                  >
                    ...
                    <span className={`text-sm font-medium md:font-semibold ${selectedClass === cls ? "text-slate-900" : "text-slate-600"}`}>
                      {cls}
                    </span>
                  </button>
                ))}
              ...
              <div className="flex gap-3">
                {Object.keys(streamSubjects).map((stream) => (
                  ...
                  >
                    ...
                    <span className={`text-xs font-medium md:font-semibold ${selectedStream === stream ? "text-slate-900" : "text-slate-600"}`}>
                      {stream}
                    </span>
                  </button>
                ))}
          ...
          <div className={`w-full border rounded-2xl p-5 md:p-6 transition-colors ${hasSubjectError ? 'bg-red-50/30 border-red-200' : 'bg-[#f6fcf8] border-emerald-100'}`}>
            <div className="flex justify-between items-center mb-5">
              <h3 className={`text-sm font-medium md:font-bold ${hasSubjectError ? 'text-red-700' : 'text-emerald-800'}`}>Subject Combinations</h3>
              {hasSubjectError && <span className="text-[10px] font-medium md:font-semibold text-red-500">Please select 4 electives</span>}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-6">
              ...
              <div className="flex flex-col gap-6">
                ...
                {selectedSubjects.length > 0 && (
                  <div>
                    <label className="text-[10px] font-medium md:font-bold text-slate-400 uppercase tracking-wider mb-2 block">
                    ...
                    <div className="flex flex-col gap-2">
                      {selectedSubjects.map((sub, i) => (
                        <div key={i} className="w-full bg-white border border-blue-100 rounded-xl p-3.5 flex items-center gap-3 shadow-sm transition-all">
                          ...
                          <span className="text-sm font-medium md:font-semibold text-slate-700 leading-tight">{sub}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {selectedSubjects.length < 5 && (
                  <div className="w-full bg-orange-50/50 border border-orange-200 border-dashed rounded-xl p-3.5 flex items-center justify-center mt-auto">
                    <span className="text-[11px] font-medium md:font-semibold text-orange-600">
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`text-sm font-normal md:font-medium ${isSelected ? "text-slate-800" : "text-slate-600"}`}>
                            {subject.name}
                          </span>
                        </div>
                        {subject.compulsory && (
                          <span className="text-[9px] font-medium md:font-semibold text-slate-400 uppercase tracking-widest">
                            COMPULSORY
                          </span>
                        )}
                      </div>
