          ...
          {steps.map((step) => {
            ...
            return (
              <div key={step.id} className="flex flex-col items-center gap-3 relative z-10 w-24">
                <div 
                  className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-medium md:font-semibold transition-colors duration-300 ${
                    isActive 
                ...
                <span className={`text-xs font-normal md:font-medium text-center leading-tight ${
      ...
      <div className="md:hidden w-full bg-white rounded-2xl border border-slate-200 p-5 shadow-sm mb-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex flex-col">
            <span className="text-[10px] font-medium md:font-bold text-brand-green tracking-wider uppercase mb-1">
              Step {currentStep} of {steps.length}
            </span>
            <h2 className="text-lg font-medium md:font-bold text-slate-800">
              {steps[currentStep - 1].title}
            </h2>
          </div>
          <div className="bg-emerald-50 text-brand-green px-3 py-1.5 rounded-full text-xs font-medium md:font-semibold border border-emerald-100 flex items-center gap-1.5">
