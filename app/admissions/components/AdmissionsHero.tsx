...
import { Laptop, UserCircle, ListChecks, CheckCircle2, ArrowRight, AlertCircle } from "lucide-react";
import { getCurrentAcademicSession } from "@/lib/sessionUtils";

interface AdmissionsHeroProps {
  onStartApplication?: () => void;
  isAdmissionsClosed?: boolean;
}

export default function AdmissionsHero({ onStartApplication, isAdmissionsClosed }: AdmissionsHeroProps) {
  const steps = [
    {
      id: 1,
  ...
  return (
    <section className="w-full bg-[#f9fafb] pt-24 md:pt-28 pb-12 px-4 md:px-8 relative overflow-hidden font-sans">
      <div className="max-w-7xl mx-auto flex flex-col">
        ...
        <div className="flex flex-col mb-6 md:mb-8">
          ...
          {/* Red Notice Line Placed Right Below Admission Process Heading */}
          {isAdmissionsClosed && (
            <div className="w-full bg-rose-600 text-white py-3.5 px-6 rounded-2xl text-center font-medium text-xs md:text-sm flex items-center justify-center gap-2.5 shadow-md shadow-rose-600/20 border border-rose-500 mt-5 animate-in fade-in duration-200">
              <AlertCircle size={18} className="shrink-0 animate-bounce" />
              <span>Notice: Online admissions for session <strong>{getCurrentAcademicSession()}</strong> are currently <strong>CLOSED</strong>. For inquiry or assistance, please contact the school office.</span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col items-center w-full mt-4">
          <span className="text-[9px] md:text-[10px] font-semibold text-brand-green uppercase tracking-[0.2em] mb-2">
            STEP-BY-STEP GUIDE &mdash; SESSION {getCurrentAcademicSession()}
          </span>
          ...
          <button 
            onClick={handleStart}
            className="mt-12 bg-brand-green hover:bg-[#0d924c] text-white px-6 py-2.5 rounded-full font-semibold text-xs md:text-sm flex items-center gap-2 transition-colors shadow-lg shadow-brand-green/20 border-0 cursor-pointer"
          >
