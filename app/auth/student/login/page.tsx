...
export default function StudentLoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (
        (cleanEmail === "alex@email.com" && password === "demo123") ||
        (cleanEmail === "demo@email.com" && password === "demo")
      ) {
        setSuccess("Login successful! Redirecting to student hub...");
        
        // Save mock student data to localStorage
      ...
      <div className="w-full max-w-[460px] bg-white border border-slate-100/80 rounded-3xl p-8 md:p-10 shadow-[0_15px_45px_rgba(0,0,0,0.015)] flex flex-col gap-6 relative">
        ...
        <div className="bg-emerald-50/30 border border-emerald-100/50 rounded-2xl p-4 flex flex-col items-center">
          ...
          <div className="flex flex-col gap-1 text-center text-xs text-slate-600 font-semibold mb-2.5">
            <div>Email: <code className="bg-emerald-50 border border-emerald-100/30 px-1.5 py-0.5 rounded text-emerald-800 font-mono">alex@email.com</code></div>
            <div>Password: <code className="bg-emerald-50 border border-emerald-100/30 px-1.5 py-0.5 rounded text-emerald-800 font-mono">demo123</code></div>
          </div>
          <button
        ...
        <div className="text-xs text-slate-500 font-medium text-center pt-2 border-t border-slate-50">
          ...
          <Link href="/auth/student/register" className="text-[#0c3c86] font-bold hover:underline">
            Sign up
          </Link>
        </div>
