...
export default function NoticesList() {
  ...
  const getCategoryBadge = (cat: Notice["category"]) => {
    switch (cat) {
      case "Academic":
        return (
          <span className="inline-flex items-center gap-1 text-[9px] md:text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md border border-brand-navy/15 bg-brand-navy/5 text-brand-navy select-none shrink-0">
            <ShieldCheck className="w-3 h-3 text-brand-navy/70 stroke-[2.5]" /> Academic
          </span>
        );
      case "Urgent":
        return (
          <span className="inline-flex items-center gap-1 text-[9px] md:text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md border border-rose-200 bg-rose-50 text-rose-600 select-none shrink-0">
            <ShieldAlert className="w-3 h-3 text-rose-500/70 stroke-[2.5]" /> Urgent
          </span>
        );
      case "Careers":
        return (
          <span className="inline-flex items-center gap-1 text-[9px] md:text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md border border-brand-green/20 bg-brand-green/5 text-brand-green select-none shrink-0">
            <Briefcase className="w-3 h-3 text-brand-green/70 stroke-[2.5]" /> Careers
          </span>
        );
      default:
        return (
          <span className="inline-flex items-center gap-1 text-[9px] md:text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md border border-slate-200 bg-slate-50 text-slate-600 select-none shrink-0">
            General
          </span>
        );
    }
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-100 pb-5">
        ...
        <div className="flex gap-2">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-full font-bold text-xs md:text-sm transition-all cursor-pointer whitespace-nowrap ${
                activeTab === cat
        ...
        <div className="relative w-full md:w-80">
          <input
            ...
            className="w-full h-10 pl-10 pr-4 rounded-xl border border-slate-200 bg-white text-xs md:text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-brand-green/20 focus:border-brand-green transition-all"
          />
      ...
      <div className="flex flex-col gap-3">
        {filteredNotices.length > 0 ? (
          filteredNotices.map((notice) => (
            ...
            >
              ...
              <div className="flex items-center gap-3 min-w-0 flex-1">
                ...
                {notice.isNew && (
                  <span className="text-[8px] font-extrabold text-brand-green bg-brand-green/5 border border-brand-green/10 px-1.5 py-0.5 rounded select-none shrink-0">
                    • NEW
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-4 md:gap-5 shrink-0 justify-end text-[10px] md:text-xs">
                ...
                <span className="text-brand-green font-extrabold flex items-center gap-0.5 group-hover:translate-x-0.5 transition-transform">
          ...
          <div className="text-center py-20 bg-white border border-slate-100 rounded-2xl flex flex-col items-center gap-3">
            ...
            <span className="text-slate-400 text-sm font-semibold">No notices match your search criteria.</span>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col gap-3.5">
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5 text-slate-800 text-xs md:text-sm font-bold leading-relaxed">
              ...
              {selectedNotice.hasAttachment && (
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-emerald-50/30 border border-emerald-100/30 rounded-2xl p-4 mt-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-700">Official_Circular_{selectedNotice.id}.pdf</span>
                  </div>
                  <button
                    onClick={() => alert(`Downloading Circular PDF for Notice ID: ${selectedNotice.id}`)}
                    className="inline-flex items-center gap-1.5 bg-brand-green text-white hover:bg-brand-green-dark px-4 py-2 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-sm shadow-emerald-500/10 border-transparent"
                  >
